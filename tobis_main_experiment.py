...
def writeData(datalist, subID):
    ...
    subject_data_file = './Data/Sub' + subID + '.csv'  # make the name for the csv file
    with open(subject_data_file, 'w', newline='') as csvfile:  # make and open the csv file
        header = ['SubjectID', 'StimulusType', 'response', 'RT']  # initialize the names for the header
        writer = csv.writer(csvfile)  # make the csvfile

        writer.writerow(header)  # make the header
        writer.writerows(datalist)  # write each entry of datalist to a single row
                    ...
                    for event in pygame.event.get():
                        ...
                        if event.type == pygame.KEYDOWN:
                            if event.key == pygame.K_SPACE:
                                ...
                                response = 1  # should be assigned 1 if K_SPACE is pressed
...
if __name__ == "__main__":
    ...
    subID = str(input("Please enter subject ID: "))
    subID_data_path = './Data/Sub' + subID + '.csv'
    if os.path.exists(subID_data_path):
        print('The subject ID already exists. Please use another subject ID.')
        quit()
